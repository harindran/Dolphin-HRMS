    ...
    Friend Class frmAdditionDeduction
        ...
        Private Sub OnCustomInitialize()
            Try
                ...
                odbdsDetails = objform.DataSources.DBDataSources.Item("@SMPR_PAD1")
        ...
        Private Sub loadcombobox()
            Try
                ...
                objrs.DoQuery("EXEC [Innova_HRMS_EMPMASTER_COMBO_FILLING] 'OPAD'")
...
#End Region
        ...
        Private Sub ComboBox2_ComboSelectAfter(sboObject As System.Object, pVal As SAPbouiCOM.SBOItemEventArg) Handles ComboBox2.ComboSelectAfter
            ...
            objrs = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            objrs.DoQuery("select Replace(convert(varchar,F_Refdate,103),'/','.')F_Refdate,Replace(convert(varchar,T_RefDate,103),'/','.')T_RefDate from OFPR where Code='" & ComboBox2.Selected.Value & "'")
        ...
        Private Sub Load_employees(ByVal strempid As String, ByVal rowno As Integer)
            Try
                ...
                objform.Freeze(True)
                Dim strsql As String = " Declare @EMPID as varchar(max) "
                strsql += vbCrLf + " set @EMPID='" & strempid & "'"
                ...
                strsql += vbCrLf + " inner join (select rowno,splitdata from [fnSplitString](RIght(@empid,len(@empid)-1),'#')) S on S.splitdata=T0.U_ExtEmpNo"
                ...
                For i = 1 To objrs.RecordCount
                    Try
                        ...
                        odbdsDetails.SetValue("U_ExtEmpNo", 0, objrs.Fields.Item("U_ExtEmpNo").Value)
                        odbdsDetails.SetValue("U_EmpID", 0, objrs.Fields.Item("U_empID").Value)
                        odbdsDetails.SetValue("U_EmpName", 0, objrs.Fields.Item("empName").Value)
                        odbdsDetails.SetValue("U_Dept", 0, objrs.Fields.Item("U_dept").Value)
                        odbdsDetails.SetValue("U_Designat", 0, objrs.Fields.Item("Desig").Value)
        ...
        Private Sub EditText4_LostFocusAfter(sboObject As Object, pVal As SAPbouiCOM.SBOItemEventArg) Handles EditText4.LostFocusAfter
            Try
                objaddon.objglobalmethods.LoadCombo_Series(objform, "cmbseries", "OPAD", IIf(EditText4.String = "", Now.Date, Date.ParseExact(EditText4.String, "dd.MM.yyyy", System.Globalization.DateTimeFormatInfo.InvariantInfo)))
            Catch ex As Exception
