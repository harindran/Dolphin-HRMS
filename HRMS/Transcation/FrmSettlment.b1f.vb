    ...
    Friend Class FrmSettlment
        ...
        Private Sub Combo_Load()
            Try
                ...
                objrs.DoQuery("EXEC [Innova_HRMS_EMPMASTER_COMBO_FILLING] 'OLSE'")
...
#End Region
        ...
        Private Sub ComboBox0_ComboSelectAfter(sboObject As Object, pVal As SAPbouiCOM.SBOItemEventArg) Handles ComboBox0.ComboSelectAfter
            ...
            EditText38.Value = objaddon.objglobalmethods.GetDocnum_BaseonSeries("OLSE", ComboBox0.Selected.Value)
        End Sub
...
#Region "Employee Details Loading "
        ...
        Private Sub Load_employeeDetails()
            ...
            Try
                ...
                strsql = "Exec [Innova_HRMS_GetEmpDetails_Settlement] '" & EditText1.Value.ToString & "'"
                objrs = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                objrs.DoQuery(strsql)
                If objrs.RecordCount = 0 Then Exit Sub
...
#Region "Leave Application Part"
        ...
        Private Sub EditText9_ChooseFromListAfter(sboObject As Object, pVal As SAPbouiCOM.SBOItemEventArg) Handles EditText9.ChooseFromListAfter
            Try
                ...
                If Not pCFL.SelectedObjects Is Nothing Then
                    Try
                        EditText9.Value = pCFL.SelectedObjects.Columns.Item("DocEntry").Cells.Item(0).Value
                    Catch ex As Exception
                    End Try
        ...
        Private Sub EditText9_LostFocusAfter(sboObject As Object, pVal As SAPbouiCOM.SBOItemEventArg) Handles EditText9.LostFocusAfter
            If EditText9.Value.ToString = "" Then Clear_LeaveAppDetails()
        End Sub
        ...
        Private Sub LeaveApplication_Filling()
            Try
                ...
                strsql = "select DocEntry,DocNum,Replace(Convert(varchar,U_FromDate,103),'/','.')[FromDate],Replace(Convert(varchar,U_Todate,103),'/','.')[ToDate],U_NoDayLve,Replace(Convert(varchar,Dateadd(dd,-1,U_FromDate),103),'/','.')[encashdate]"
                strsql += vbCrLf + " ,replace(convert(varchar,U_RejoinDt,103),'/','.')[Rejoin]  from [@SMPR_OLVA] where Docentry='" & EditText9.Value & "' and U_Approved='Y' and status='O'"
                objrs = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                objrs.DoQuery(strsql)
                If objrs.RecordCount = 0 Then Exit Sub
...
#Region "Leave Encashment Part"
        ...
        Private Sub Encashment_eligibilty_Leave_Calcualtion()
            ...
            strsql = "Exec [Innova_HRMS_LeaveApplication_Balance] '" & EditText15.Value & "','" & EditText2.Value & "','AL'"
...
#Region "Air Ticket Part"
        ...
        Private Sub AirTicketIssue_Details_Filling()
            Try
                ...
                strsql = "Select DocNum,Replace(convert(Varchar,U_TickDate,103),'/','.')[TicketDate],U_eligiamt,U_noofday,U_Total from [@SMPR_OTIS] where Docentry='" & EditText21.Value & "'"
                objrs = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                objrs.DoQuery(strsql)
                If objrs.RecordCount = 0 Then Exit Sub
...
#Region "Advance Salary Splitup"
        ...
        Private Sub Button2_ClickAfter(sboObject As Object, pVal As SAPbouiCOM.SBOItemEventArg) Handles Button2.ClickAfter
            Try
                ...
                strsql = "select isnull(convert(varchar,max(U_todate),103),'')[Todate] from [@SMPR_OPRC] where isnull(U_Process ,'N')='Y' and U_todate>='" & EditText25.Value & "'"
                objrs = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                objrs.DoQuery(strsql)
                ...
                For i As Integer = 0 To 100
                    ...
                    Matrix0.Columns.Item("fromdt").Cells.Item(Matrix0.VisualRowCount).Specific.string = Fromdate.ToString("dd.MM.yyyy")

                    Matrix0.Columns.Item("todt").Cells.Item(Matrix0.VisualRowCount).Specific.string = LastDayInMonthDate.ToString("dd.MM.yyyy")
        ...
        Private Sub loanDetails_loading()
            Try
                ...
                For i As Integer = 1 To objrs.RecordCount
                    ...
                    Matrix2.Columns.Item("date").Cells.Item(Matrix2.VisualRowCount).Specific.string = objrs.Fields.Item("Date").Value.ToString
...
#Region "Gratuity"
        ...
        Private Sub Load_Gratuity()
            Try
                ...
                strsql = "EXEC [Innnova_HRMS_Grauity_Settlement] '" & EditText0.Value & "','" & EditText2.Value & "'"
                objrs1 = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                objrs1.DoQuery(strsql)
...
#Region "Pay Details"
        ...
        Private Sub Load_Paydetails()
            Try
                ...
                strsql = " select T1.U_PayElCod,T1.U_PayElNam,T1.U_Amount from [@SMPR_OHEM] T0 inner join [@SMPR_HEM1] T1 on T0.code=T1.code where isnull(U_LveSettlement,'N')='Y' and  isnull(T0.U_empid,'')='" & EditText2.Value & "'"
...
#End Region
        ...
        Private Sub EditText36_LostFocusAfter(sboObject As Object, pVal As SAPbouiCOM.SBOItemEventArg) Handles EditText36.LostFocusAfter
            Try
                objaddon.objglobalmethods.LoadCombo_Series(objform, "cmbseries", "OLSE", IIf(EditText36.String = "", Now.Date, Date.ParseExact(EditText36.String, "dd.MM.yyyy", System.Globalization.DateTimeFormatInfo.InvariantInfo)))
            Catch ex As Exception
            End Try
        End Sub
