    ...
    Friend Class frmLeaveApplicaiton
        ...
        Private Sub FieldSettings()
            Try
                ...
                'objaddon.objglobalmethods.SetAutomanagedattribute_Editable(objform, "txtlstart", True, True, False)
                'objaddon.objglobalmethods.SetAutomanagedattribute_Editable(objform, "txtlend", True, True, False)
...
#End Region
            ...
            Try
                ...
                If Not pCFL.SelectedObjects Is Nothing Then
                    ...
                    Try
                        ...
                        Dim strsql As String = " select U_ExtEmpNo,isnull(U_jobTitle,'')[Desig],isnull(T1.name,'')[Dept],isnull(T2.location,'')[Location],isnull(T0.U_mobile,'')[Local],isnull(T0.U_pager,'')[Native] from [@SMPR_OHEM] T0 "
                        strsql += vbCrLf + " left join OUDP T1 on T0.U_dept=T1.code left join olct T2 on T2.code=T0.U_location  where  U_ExtEmpNo='" & EditText0.Value & "'"
                        objrs = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                        objrs.DoQuery(strsql)
        ...
        Private Sub LoadHistory_leavedetails()
            ...
            strsql = "Exec [Innova_HRMS_LeaveApplicaiton_History] '" & EditText1.Value & "','" & EditText10.Value & "','" & EditText23.Value & "'"
            'objrs = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            'objrs.DoQuery(strsql)
            'If objrs.RecordCount = 0 Then Exit Sub
        ...
        Private Sub LeaveCalculation()
            ...
            If EditText7.String <> "" And EditText12.String <> "" Then
                Dim Fromdate As Date = Date.ParseExact(EditText7.String, "dd.MM.yyyy", System.Globalization.DateTimeFormatInfo.InvariantInfo)
                Dim todate As Date = Date.ParseExact(EditText12.String, "dd.MM.yyyy", System.Globalization.DateTimeFormatInfo.InvariantInfo)
                ...
                objform.Items.Item("txtrejoin").Specific.string = DateAdd(DateInterval.Day, 1, todate).ToString("dd.MM.yyyy")
            Else
                EditText13.Value = 0
            End If
            ...
            If EditText10.Value = "" Or EditText1.Value = "" Or EditText7.Value = "" Then
                EditText14.Value = 0
            Else
                strsql = "Exec [Innova_HRMS_LeaveApplication_Balance] '" & EditText7.Value & "','" & EditText1.Value & "','" & EditText10.Value & "'"
                objrs = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                objrs.DoQuery(strsql)
...
FIeldDisable:
        ...
        Private Sub EditText9_LostFocusAfter(sboObject As Object, pVal As SAPbouiCOM.SBOItemEventArg) Handles EditText9.LostFocusAfter
            Try
                objaddon.objglobalmethods.LoadCombo_Series(objform, "cmbseries", "OLVA", IIf(EditText9.String = "", Now.Date, Date.ParseExact(EditText9.String, "dd.MM.yyyy", System.Globalization.DateTimeFormatInfo.InvariantInfo)))
            Catch ex As Exception
