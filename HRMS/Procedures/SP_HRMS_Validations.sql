...
BEGIN 
	...
	IF EXISTS (SELECT 1 where isnull(@todate,@fromdate) <@fromdate) 
	Begin SET @error = '1201' SET @error_message = 'Account Determination To Date should be greater than fromdate' RETURN @error End
...
BEGIN
	...
	if exists(SELECT  1 FROM #temp_OLOA WHERE convert(varchar,isnull(Series,''))='') Begin select @error = '40001',  @error_message = 'Series is Missing' Return @error End
	if exists(Select 1 FROM #temp_OLOA WHERE Convert(date,ISNULL(U_EffDate,'20000101'))<convert(date,getdate()) AND @transaction_type='A') 	Begin 
	SELECT  @error = '40001',  @error_message = 'Effective Date Should be Greater than today date' Return @error end
...
BEGIN
	...
	if exists (Select 1 from #SMPR_ODAS  where Location='28' and usersign<>'1' and Transtype='A') Begin select @error = 5100,@error_message= 'Manual Attendance Posting for DIC is not allowed.'  Return @error End
	...
	set @details=''
	Select @details=coalesce
	(@details+',','')+Convert(varchar,T1.LineId ) From #SMPR_ODAS T1 where isnull(T1.U_AttStatus,'')=isnull(U_HalfStatus,'')
...
BEGIN
	...
	Select T0.DocEntry,T0.Series,T0.U_PayPerid,T0.U_FromDate,T0.U_todate,T0.U_EmployeeStatus,T0.U_docdate,T0.U_Location,T1.U_empid 
	into #Temp_payrol from [@SMPR_OPRC] T0 inner join [@SMPR_PRC1] T1 on T0.docentry=T1.docentry where T0.docentry=@docentry
